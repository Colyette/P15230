...
class ADXL345
{
	private:
		...
		int zG[3];
		int readRegister(uint8_t reg_addr, int nbytes, uint8_t *buffer);
		int writeRegister(uint8_t reg_addr, int nbytes, uint8_t *buffer);
  
        ADXL345(); //restrict from initialization without dev handle
        int dev_handle;
	public:
		enum range_t
		{
        ...
        // \precon the dev_handle is already open
		ADXL345(int e_dev_handle);
